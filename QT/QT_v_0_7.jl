...
function make_candidatecluster!(distanceMatrix::Array{Float64,2},threshold,candidateclustermatrix::Array{Int64,2},candidatepoints::Array{Int64,1})
	...
	while length(candidatepoints) > 0 
		...
		#I have considered adding a sorted set instead or similar 
		#if argmin(candidateCluster)== 1000||  argmin(candidateCluster)== 1000 || argmin(candidateCluster)== 1000
		#	#flag = check_alreadymade_cluster(candidateclustermatrix,candidateCluster)
		#	if check_alreadymade_cluster(candidateclustermatrix,candidateCluster) 
...
#This implementation is slooow 
function check_alreadymade_cluster(outcluster,currentout)

	knownsize = argmin(currentout)-1
	knownlist =  sort(currentout[1:knownsize])

	for i in 1:size(outcluster,2)
		item = view(outcluster,:,i)
		size = argmin(item)-1
		if knownsize <= size
			if sort(item[1:knownsize]) == knownlist
				return(true)
			end
		end	
	end
...
function qt(filename, inputargument)
	...
	#acceptancelist = Acceptancelist(zeros(Int64,obnum),Array{Int64}(1:1:obnum),Array{Int64}(1:1:obnum))
	greylist = update_greylist(Array{Int64}(1:1:obnum),distanceMatrix,Array{Int64}(1:1:obnum))

	acceptancelist = Acceptancelist(zeros(Int64,1),greylist,Array{Int64}(1:1:obnum))
...
function recycle_clusters(potentialclusterlist,finishCandidateCluster,candidateClusterMatrix,candidateClusterDistance)
	#note that this one, also will remove the finished cluster
...
function  update_colorlist(acceptancelist,recalculatelist,finishCandidateCluster,distanceMatrix)
	...
	greylist = update_greylist(greylist,distanceMatrix,whitelist)

	return(Acceptancelist(blacklist,greylist,whitelist))
end

#This implementation, was hoped to be more effective than the other, but still not good enough. 
function update_greylist(greylist,distanceMatrix,whitelist)
	

	return greylist
	#######
	#This method is idioti, shall be delated later. 
	#######

	#This part could also be made in a dict, i would presume.
	#Since my though is to run through the "bin" (even if it is a dict or vector), the vector is fasted. 

	#CHANGE THE NUMBERS FROM THE DISTANCEMATRIX VIEW FROM WHITELIST, TO POTENTIAL LIST

	checklist = Vector{Array{Int64,1}}()
	greylistout = Array{Int64,1}()

	for seed in greylist
		item = view(distanceMatrix,whitelist,seed)
		item = sort(partialsortperm(item, 1:3))
		#println(seed, "\t", item)

		checkflag = false
		if length(item)>2
			for stuff in checklist
				if stuff == item  
					#println(item,"\t",stuff)
					checkflag = true
					break 
